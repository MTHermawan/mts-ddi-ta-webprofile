    ...
    try {
        ...
        if ($total_count > 250) {
            ...
            foreach ($data as $key => $foto_galeri) {
                if (
                    ($id_foto_galeri !== null && $foto_galeri['id_foto_galeri'] != $id_foto_galeri) ||
...
function UpdateFotoGaleri($id_foto_galeri, $judul_foto_galeri, $deskripsi_foto_galeri, $file_foto)
{
    ...
    try {
        // Mengambil data lama
        if (!($old_data = GetFotoGaleri(id_foto_galeri: $id_foto_galeri)))
...
function DeleteFotoGaleri($id_foto_galeri)
{
    ...
    $success = false;
    try {
        if (!($data = GetFotoGaleri(id_foto_galeri: $id_foto_galeri)))
            throw new Exception("Data foto galeri tidak ditemukan");
